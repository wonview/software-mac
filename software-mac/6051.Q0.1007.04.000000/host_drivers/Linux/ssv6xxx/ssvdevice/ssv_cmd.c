...
// Freddie ToDo: Dump struct ssv6xxx_cfg
struct ssv6xxx_cfg_cmd_table cfg_cmds[] = {
    ...
    { "def_chan",           (void *)&ssv_cfg.def_chan,     0,   __string2u8         },
    ...
    { "use_wpa2_only",      (void *)&ssv_cfg.use_wpa2_only, 0,  __string2u8         },
    { "wifi_tx_gain_level_gn",(void *)&ssv_cfg.wifi_tx_gain_level_gn, 0,   __string2u8   },
    { "wifi_tx_gain_level_b", (void *)&ssv_cfg.wifi_tx_gain_level_b, 0,   __string2u8   },
    { NULL, NULL, 0, NULL },
};
...
static int ssv_cmd_mib(int argc, char *argv[])
{
    ...
    if (argc==2 && !strcmp(argv[1], "reset") ) {
        ...
        //sprintf(temp_str, " => write [0x%08x]: 0x%08x\n", 
        //    addr, value);
        //strcat(ssv6xxx_result_buf, temp_str);
...
//         sprintf(temp_str, "MRX_MB_MISS        MRX_NIDLE_MISS     DBG_LEN_ALC_FAIL    DBG_LEN_CRC_FAIL\n");
         strcat(ssv6xxx_result_buf, temp_str);
 ...
 //         sprintf(temp_str, "DBG_AMPDU_PASS      DBG_AMPDU_FAIL      ID_ALC_FAIL1        ID_ALC_FAIL2\n");
          strcat(ssv6xxx_result_buf, temp_str);
...
static int ssv_cmd_iqk (int argc, char *argv[]) {
    ...
    char temp_str[512], *endp;
    //SKip defined but not used  for compiler.
    struct sk_buff      *skb;
    struct cfg_host_cmd *host_cmd;
    u32 rxcnt_total, rxcnt_error;
    // double rxcnt_error_percet;
    ...
    if ((argc == 3) && (strcmp(argv[1], "cfg-pa") == 0)) {
        ...
        // rxcnt_error_percet = (double)rxcnt_error*100 / rxcnt_total;

        // sprintf(temp_str,"## GN mode Rx error rate = (%06d/%06d) = %4.4f\n", rxcnt_error, rxcnt_total, rxcnt_error_percet);
        ...
        // sprintf(temp_str,"## B Rx error rate = (%06d/%06d) = %4.4f\n", rxcnt_error, rxcnt_total, rxcnt_error_percet);
...
static int txtput_thread_m2(void *data)
{
    ...
    while (!kthread_should_stop() && ssv6xxx_txtput->loop_times > 0) {     
    	...
    	if(skb == NULL) {
    		printk("ssv command txtput_generate_m2 ssvdevice_skb_alloc fail!!!\n");
    		return 0;
    	}
    	...
    	tx_desc->reason = ID_TRAP_SW_TXTPUT;//= ID_TRAP_SW_TXTPUT;
...
static int txtput_thread(void *data)
{
    ...
    while (!kthread_should_stop() && ssv6xxx_txtput->loop_times > 0) {     
        ...
        //printk("free_tx_page %d, send_cnt %d, occupied_tx_pages %d\n", 
        //            free_tx_page, send_cnt, ssv6xxx_txtput->occupied_tx_pages);
