...
#ifdef CONFIG_SSV_CABRIO_E
	if (sh->cfg.ipd)
		memcpy(phy_info_tbl, phy_info_ipd, sizeof(phy_info_ipd));
#endif
#ifdef SSV6200_ECO
...
static int ssv6xxx_init_hw(struct ssv_hw *sh)
{
	...
	for (i = 0;
	     ...
	     i++) {
		if (sh->cfg.ipd) {
			if (sh->cfg.internal_volt == SSV6XXX_VOLT_33V) {
				if (ssv6200_rf_tbl[i].address == 0xCE010008) {
					ssv6200_rf_tbl[i].data = 0x004D561C;
				}
			} else {
				printk("Please redefine internal_volt(3.3V) "
				       "for IPD \n");
				BUG_ON(1);
			}
			...
			if (sh->cfg.internal_volt == SSV6XXX_VOLT_42V) {
				if (ssv6200_rf_tbl[i].address == 0xCE010008) {
					ssv6200_rf_tbl[i].data = 0x000DF69B;
				}
			} else if (sh->cfg.internal_volt == SSV6XXX_VOLT_33V) {
				if (ssv6200_rf_tbl[i].address == 0xCE010008) {
					ssv6200_rf_tbl[i].data = 0x004DF61B;
				}
			} else {
				printk("Please redefine internal_volt(3.3V or "
				       "4.2V) \n");
				BUG_ON(1);
			}
			if (ssv6200_rf_tbl[i].address == 0xCE010014) {
	...
	if (sh->cfg.ipd)
		printk("Define SSV_IPD(SSV6051Z.SSV6060P)\n");
	else
	...
	printk("TX power setting 0x%x\n",
	       ((wifi_tx_gain[sh->cfg.wifi_tx_gain_level_b] & 0x0000ffff) |
		(wifi_tx_gain[sh->cfg.wifi_tx_gain_level_gn] & 0xffff0000)));
	init_iqk_cfg.cfg_def_tx_scale_11b =
	    (wifi_tx_gain[sh->cfg.wifi_tx_gain_level_b] >> 0) & 0xff;
	init_iqk_cfg.cfg_def_tx_scale_11b_p0d5 =
	    (wifi_tx_gain[sh->cfg.wifi_tx_gain_level_b] >> 8) & 0xff;
	init_iqk_cfg.cfg_def_tx_scale_11g =
	    (wifi_tx_gain[sh->cfg.wifi_tx_gain_level_gn] >> 16) & 0xff;
	init_iqk_cfg.cfg_def_tx_scale_11g_p0d5 =
	    (wifi_tx_gain[sh->cfg.wifi_tx_gain_level_gn] >> 24) & 0xff;
...
static int ssv6xxx_read_configuration(struct ssv_hw *sh)
{
	...
	if (ssv_cfg.ipd < 2)
		sh->cfg.ipd = ssv_cfg.ipd;
	else {
		printk("Please redefine ipd(wifi.cfg)!!\n");
#ifdef CONFIG_SSV_SUPPORT_ANDROID
		BUG_ON(1);
#endif
		return 1;
	}
	if (ssv_cfg.internal_volt == 33)
		sh->cfg.internal_volt = SSV6XXX_VOLT_33V;
	else if (ssv_cfg.internal_volt == 42)
		sh->cfg.internal_volt = SSV6XXX_VOLT_42V;
	else {
		printk("Please redefine internal_volt(wifi.cfg)!!\n");
#ifdef CONFIG_SSV_SUPPORT_ANDROID
		BUG_ON(1);
#endif
		return 1;
	...
	sh->cfg.software_mac_mode = ssv_cfg.software_mac_mode;
	return 0;
}
