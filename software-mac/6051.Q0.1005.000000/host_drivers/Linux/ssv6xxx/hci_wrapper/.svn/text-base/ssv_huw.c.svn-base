...
static long ssv_huw_ioctl_process(struct ssv_huw_dev *glue, unsigned int cmd, struct ssv_huw_cmd *pucmd_data, bool isCompat)
{
    ...
    //printk("[HCI user-space wrapper]: %s:%d\n", __FUNCTION__, __LINE__);
...
static ssize_t ssv_huw_read(struct file *fp, char __user * buf, size_t length, loff_t * offset)
{
    ...
    if (skb != NULL)
    {
        ...
        dev_kfree_skb_any(skb);
        //dev_err(g_huw_dev.dev,"%s():get rx data : data len:[%d], user read len:[%d],real read len:[%d] \n"
        //    , __FUNCTION__, skb->len, (u32)length, copy_length);
    }
...
static ssize_t ssv_huw_write(struct file *fp, const char __user * buf, size_t length, loff_t * offset)
{
    ...
    //int i = 0;
    len = (len & 0x1f)?(((len>>5) + 1)<<5):len; //alignment
...
{    
    ...
    //INIT_LIST_HEAD(&hdev->rxbuf);
    //INIT_LIST_HEAD(&hdev->rxreadybuf);
...
int ssv_huw_probe(struct platform_device *pdev)
{
    ...
    //Register to HCI to communicate with
    ...
    //create character device for HCI/SDIO middle layer
