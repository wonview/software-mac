...
int ssv6xxx_init_mac(struct ssv_hw *sh)
{
    ...
    /* Reset MAC before initialize it. */
...
static int ssv6xxx_init_hw(struct ssv_hw *sh)
{
    ...
    */
    ...
    for(i=0; i<sizeof(ssv6200_rf_tbl)/sizeof(struct ssv6xxx_dev_table); i++)
    {
        ...
        {
            if(sh->cfg.crystal_frequecy_offse)
            {
                ssv6200_rf_tbl[i].data &= RG_XOSC_CBANK_XO_I_MSK;
                ssv6200_rf_tbl[i].data |= (sh->cfg.crystal_frequecy_offse << RG_XOSC_CBANK_XO_SFT);
            }
        }
    }
    ...
    init_iqk_cfg.cfg_papd_tx_scale_11b = (wifi_tx_gain[sh->cfg.wifi_tx_gain_level_b]>>0) & 0xff;
    init_iqk_cfg.cfg_papd_tx_scale_11b_p0d5 = (wifi_tx_gain[sh->cfg.wifi_tx_gain_level_b]>>8) & 0xff;
    init_iqk_cfg.cfg_papd_tx_scale_11g = ((wifi_tx_gain[sh->cfg.wifi_tx_gain_level_gn]>>16) & 0xff)+0x30;
    init_iqk_cfg.cfg_papd_tx_scale_11g_p0d5 = ((wifi_tx_gain[sh->cfg.wifi_tx_gain_level_gn]>>24) & 0xff)+0x30;
