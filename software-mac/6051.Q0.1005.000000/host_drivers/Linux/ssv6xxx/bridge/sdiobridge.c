 ...
 *
 * Copyright (C) 2013 South Silicon Valley Inc.
 *
...
//#include <linux/ssvcabrio_platform.h>
...
//#include "ssvcabrio_platform.h"
//#include "../include/sdio_def.h"
...
//        dev_err(glue->dev,"%s: ADR[%08X] VAL[%02X] ret:[%d]\n",__FUNCTION__,address,data,ret);
...
struct file_operations fops =
{
    ...
    .read = ssv_sdiobridge_device_read,                 //Recv
    .write = ssv_sdiobridge_device_write,               //Xmit
...
//    dev_err(glue->dev, "%s:[%02X][%02X][%02X][%02X]\n",__FUNCTION__,
//            sdio_readb(func, REG_INT_MASK, &err_ret),sdio_readb(func, REG_INT_STATUS, &err_ret),
//            sdio_readb(func, REG_CARD_PKT_LEN_0, &err_ret),sdio_readb(func, REG_CARD_PKT_LEN_1, &err_ret));
...
static void ssv_sdiobridge_irq_disable(struct ssv_sdiobridge_glue *glue,bool iswaitirq)
{
    ...
    {
        func = dev_to_sdio_func(glue->dev);
        //mask = ssv_sdiobridge_irq_getmask(child);
        //ssv_sdiobridge_irq_setmask(child,0);
        sdio_claim_host(func);
        ...
        //ssv_sdiobridge_irq_setmask(child,mask);
    }
}
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,4,0)//0x030500
