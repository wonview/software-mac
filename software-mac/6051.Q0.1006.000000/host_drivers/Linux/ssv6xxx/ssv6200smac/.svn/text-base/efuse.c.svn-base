...
static u8 read_efuse(struct ssv_hw *sh, u8 *pbuf)
{
    ...
    SMAC_REG_READ(sh,SSV_EFUSE_RAW_DATA_BASE,pointer32);
    if (*pointer32 == 0x00) {
		return 0;
    }
    ...
    {
        SMAC_REG_WRITE(sh,SSV_EFUSE_READ_SWITCH+i,0x1);
        SMAC_REG_READ(sh,SSV_EFUSE_RAW_DATA_BASE+i,pointer32);
    }
    return 1;
}
...
void efuse_read_all_map(struct ssv_hw *sh)
{
    ...
    sh->cfg.crystal_frequecy_offse = ssv_cfg.crystal_frequecy_offse;
