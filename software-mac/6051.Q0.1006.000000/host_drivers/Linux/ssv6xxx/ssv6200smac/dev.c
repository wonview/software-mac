...
int ssv6xxx_skb_encrypt(struct sk_buff *mpdu, struct ssv_softc *sc)
{
    ...
    {
        dev_err(sc->dev, "[Local Crypto]: mpdu[0x%x] Encrypt %c %d %02X:%02X:%02X:%02X:%02X:%02X with NULL crypto.\n",
                mpdu,unicast ? 'U' : 'B', mpdu->protocol,
                hdr->addr1[0], hdr->addr1[1], hdr->addr1[2],
                hdr->addr1[3], hdr->addr1[4], hdr->addr1[5]);
        return -1;
    }
...
static void _proc_data_rx_skb (struct ssv_softc *sc, struct sk_buff *rx_skb)
{
    ...
    */
    ...
    if (rxdesc->rate_idx < SSV62XX_G_RATE_INDEX && rxphypad->RSVD == 0)
    {
        ...
        {
            if((rxdesc->wsid < SSV_RC_MAX_HARDWARE_SUPPORT) && (rxdesc->RxResult == 0x2b) && sc->wsid_rssi[rxdesc->wsid])
                rxphypad->rpci = sc->wsid_rssi[rxdesc->wsid];
            ...
            printk("beacon %02X:%02X:%02X:%02X:%02X:%02X rxphypad-rpci=%d\n",
                   hdr->addr2[0], hdr->addr2[1], hdr->addr2[2],
                   hdr->addr2[3], hdr->addr2[4], hdr->addr2[5], rxphypad->rpci);
#endif
        }
        else if(ieee80211_is_probe_resp(hdr->frame_control))
        {
#if 0 /*For debug*/
            printk("probe_resp %02X:%02X:%02X:%02X:%02X:%02X rxphypad-rpci=%d\n",
                   hdr->addr2[0], hdr->addr2[1], hdr->addr2[2],
                   hdr->addr2[3], hdr->addr2[4], hdr->addr2[5], rxphypad->rpci);
#endif
            if(rxphypad->rpci > 88)
        ...
        else if(ieee80211_is_data(hdr->frame_control) && (rxdesc->wsid < SSV_RC_MAX_HARDWARE_SUPPORT))
        {
            if(sc->wsid_rssi[rxdesc->wsid])
            {
                sc->wsid_rssi[rxdesc->wsid] = (rxphypad->rpci + ((sc->wsid_rssi[rxdesc->wsid]<<3) - sc->wsid_rssi[rxdesc->wsid])) >> 3;
                rxphypad->rpci = sc->wsid_rssi[rxdesc->wsid];
            }
            else
                sc->wsid_rssi[rxdesc->wsid] = rxphypad->rpci;
            //printk("rxphypad signal %d\n",rxphypad->rpci);
        }
    ...
    else if (rxdesc->rate_idx >= SSV62XX_G_RATE_INDEX && rxphy->service == 0)
    {
        ...
        {
            if((rxdesc->wsid < SSV_RC_MAX_HARDWARE_SUPPORT) && (rxdesc->RxResult == 0x2b) && sc->wsid_rssi[rxdesc->wsid])
                rxphy->rpci = sc->wsid_rssi[rxdesc->wsid];

            if(rxphy->rpci > 88)
                rxphy->rpci = 88;
#if 0 /*For debug*/
            printk("beacon %02X:%02X:%02X:%02X:%02X:%02X rxphy-rpci=%d\n",
                   hdr->addr2[0], hdr->addr2[1], hdr->addr2[2],
                   hdr->addr2[3], hdr->addr2[4], hdr->addr2[5], rxphy->rpci);
#endif
        }
        else if(ieee80211_is_probe_resp(hdr->frame_control))
        {
#if 0 /*For debug*/
            printk("probe_resp %02X:%02X:%02X:%02X:%02X:%02X rxphy-rpci=%x\n",
                   hdr->addr2[0], hdr->addr2[1], hdr->addr2[2],
                   hdr->addr2[3], hdr->addr2[4], hdr->addr2[5], rxphy->rpci);
#endif
            if(rxphy->rpci > 88)
        ...
        else if(ieee80211_is_data(hdr->frame_control) && (rxdesc->wsid < SSV_RC_MAX_HARDWARE_SUPPORT))
        {
            if(sc->wsid_rssi[rxdesc->wsid])
            {
                sc->wsid_rssi[rxdesc->wsid] = (rxphy->rpci + ((sc->wsid_rssi[rxdesc->wsid]<<3) - sc->wsid_rssi[rxdesc->wsid])) >> 3;
                rxphy->rpci = sc->wsid_rssi[rxdesc->wsid];
            }
            else
                sc->wsid_rssi[rxdesc->wsid] = rxphy->rpci;
            //printk("rxphy signal %d\n",rxphy->rpci);
        }
...
#endif
        if((rxdesc->wsid < SSV_RC_MAX_HARDWARE_SUPPORT) && (rxdesc->RxResult == 0x2b))
