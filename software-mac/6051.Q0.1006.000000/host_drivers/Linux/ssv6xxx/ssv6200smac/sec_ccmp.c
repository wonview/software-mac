...
#ifdef MULTI_THREAD_ENCRYPT
    u8 pre_rx_pn[CCMP_PN_LEN]; //Record the rxpn while enqueue the rx packets to avoid replaying recevied packets
#endif
...
static int lib80211_ccmp_encrypt(struct sk_buff *skb, int hdr_len, void *priv)
{
	...
	//printk("prepared skb because of beef!!\n");
...
static int lib80211_ccmp_decrypt(struct sk_buff *skb, int hdr_len, void *priv)
{
	...
	// Freddie ToDo: Disable replay check temporarily.
#if 0
    ...
    /*else
    {
        printk("[MT-CRYPTO]:CCMP: overlap decrypt:rx_pn = "
            "%02x:%02x:%02x:%02x:%02x:%02x, current rx_pn = "
            "%02x:%02x:%02x:%02x:%02x:%02x\n", key->rx_pn[0], key->rx_pn[1], 
               key->rx_pn[2], key->rx_pn[3], key->rx_pn[4], key->rx_pn[5], pn[0], 
...
static int lib80211_ccmp_decrypt_prepare (struct sk_buff * skb, int hdr_len, void *priv)
{
    ...
    {
#if 1 //def CONFIG_LIB80211_DEBUG
		//if (net_ratelimit()) 
        {
			printk(KERN_DEBUG "CCMP: replay detected: STA=%pM "
				 "previous PN %02x%02x%02x%02x%02x%02x "
...
//static void ccmp_special_blocks(struct PKT_Info_st *ppkt, struct sk_buff *skb, u8 *pn, u8 *scratch, int encrypted)
static void ccmp_special_blocks(struct sk_buff *skb, u8 *pn, u8 *scratch, int encrypted)
{
	...
	mask_fc = hdr->frame_control;//*(u16*)((u8 *)ppkt + ppkt->hdr_offset);
	
	//qos_tid = ppkt->priority;
	...
	//mask_fc &= ~cpu_to_le16(IEEE80211_FCTL_RETRY | IEEE80211_FCTL_PM | IEEE80211_FCTL_MOREDATA);
	...
	//data_len = ppkt->len - ppkt->hdr_len;
	if (encrypted)
	{
	...
	//memcpy(&b_0[2], (u8 *)ppkt + ppkt->hdr_offset + 10, 6);
	...
	//memcpy(&aad[2], &mask_fc,2);
	memcpy(&aad[4], &hdr->addr1, 3 * ETH_ALEN);
	//memcpy(&aad[4], (u8 *)ppkt + ppkt->hdr_offset+4, 3*6 + 2);
	...
	if (a4_included) {
		memcpy(&aad[24], hdr->addr4, ETH_ALEN);
		//memcpy(&aad[24], (u8 *)ppkt + ppkt->hdr_offset+24, ETH_ALEN);
		aad[30] = qos_tid;
		aad[31] = 0;
	} else {
...
//int ieee80211_crypto_ccmp_encrypt(struct PKT_Info_st *ppkt,struct sk_buff *skb, u8 *key, u8 keyidx, u8 *pn)
int ieee80211_crypto_ccmp_encrypt(struct sk_buff *skb, u8 *key, u8 keyidx, u8 *tx_pn)
{
    ...
    data_len = skb->len - hdrlen;//ppkt->len - ppkt->hdr_len;
	data = ((u8*)skb->data)+hdrlen;//(u8*)ppkt + ppkt->payload_offset;
...
#if 0//proteck bit setting by HCI
	//hdr->frame_control |= cpu_to_le16(IEEE80211_FCTL_PROTECTED);
	...
	*frame |= IEEE80211_FCTL_PROTECTED;
	...
	//pos += CCMP_HDR_LEN;
	//ccmp_special_blocks(ppkt,skb, pn, crypto_buf, 0);
	...
	//memmove(data+CCMP_HDR_LEN, data, data_len);

	ccmp_pn2hdr(data+hdrlen, keyidx, pn); //Give CCMP Header addr
	...
	//ppkt->len = ppkt->len + CCMP_MIC_LEN + CCMP_HDR_LEN;
	//skb->len = skb->len + CCMP_MIC_LEN;
#ifdef SECURITY_DUMP
