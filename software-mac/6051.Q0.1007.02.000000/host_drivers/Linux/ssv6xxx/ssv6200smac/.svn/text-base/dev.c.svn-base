    ...
    if (rxdesc->rate_idx < SSV62XX_G_RATE_INDEX && rxphypad->RSVD == 0)
    {
        if((ieee80211_is_beacon(hdr->frame_control))||(ieee80211_is_probe_resp(hdr->frame_control)))
        {
            ...
            if(sta)
            {
                ...
                {
                    sc->wsid_rssi[rxdesc->wsid] = ((rxphypad->rpci << RSSI_DECIMAL_POINT_SHIFT)
                        + ((sc->wsid_rssi[rxdesc->wsid]<<RSSI_SMOOTHING_SHIFT) - sc->wsid_rssi[rxdesc->wsid])) >> RSSI_SMOOTHING_SHIFT;
                    rxphypad->rpci = (sc->wsid_rssi[rxdesc->wsid] >> RSSI_DECIMAL_POINT_SHIFT);
                }
                else
                    sc->wsid_rssi[rxdesc->wsid] = (rxphypad->rpci << RSSI_DECIMAL_POINT_SHIFT);
    ...
    else if (rxdesc->rate_idx >= SSV62XX_G_RATE_INDEX && rxphy->service == 0)
    {
        if((ieee80211_is_beacon(hdr->frame_control))||(ieee80211_is_probe_resp(hdr->frame_control)))
        {
            ...
            if(sta)
            {
                ...
                if(sc->wsid_rssi[rxdesc->wsid])
                {
                    sc->wsid_rssi[rxdesc->wsid] = ((rxphy->rpci << RSSI_DECIMAL_POINT_SHIFT)
                        + ((sc->wsid_rssi[rxdesc->wsid]<<RSSI_SMOOTHING_SHIFT) - sc->wsid_rssi[rxdesc->wsid])) >> RSSI_SMOOTHING_SHIFT;
                    rxphy->rpci = (sc->wsid_rssi[rxdesc->wsid] >> RSSI_DECIMAL_POINT_SHIFT);
                }
                else
                    sc->wsid_rssi[rxdesc->wsid] = (rxphy->rpci << RSSI_DECIMAL_POINT_SHIFT);
#ifdef SSV_RSSI_DEBUG
        ...
        // Encrypted RX packet must be for connected STA.
        if(sta)
            rxs->signal = -(sc->wsid_rssi[rxdesc->wsid] >> RSSI_DECIMAL_POINT_SHIFT);
