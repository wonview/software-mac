                 ...
                 struct ieee80211_tx_rate_control *txrc)
{
    ...
    #if 0
    // tx_info->control.vif is set after get rate in ieee80211_beacon_get_tim.
    // Can't use it to check if in p2p type.
    ...
    if (rate_control_send_low(sta, priv_sta, txrc))
    {
...
#if 1
        // Ensure the low rate assigned in rate_control_send_low is in rate index mask.
        if ((txrc->rate_idx_mask & (1 << rates[0].idx)) == 0)
        {
            ...
            {
                // No rate index found in rate_idx_mask. Use returned index.
...
//Reference rate from conntected sta.
        ...
        if (spinfo->real_hw_index != rc_rate->hw_rate_idx)
        {
            ...
            //printk("[RC] %d(%d-%d) - %s (%d - %d) %d\n", sta_priv->sta_idx, rc_sta->rc_wsid, rc_sta->ht_rc_type, string, rc_sta->rc_valid, rateidx, check_point);
        }
#endif
    ...
    /**
     * Because only single-rate is supported, we use rates[SSV_SSV_DRATE_IDX] as
     * the hardware rate index for data frames while rates[SSV_CRATE_IDX] as
...
//    printk("%s(): drate_idx=%d, crate_idx=%d\n", __FUNCTION__, rates[SSV_DRATE_IDX].count, rates[SSV_CRATE_IDX].count);
}
...
static void *ssv6xxx_rate_alloc_sta(void *priv, struct ieee80211_sta *sta, gfp_t gfp)
{
    ...
    return NULL;
#else
    sta_priv->rc_idx = (-1); // (-1) to mark RC info uninitialized.
    return sta_priv;
#endif
}
...
*/
...

    {
        ...
        if (spinfo->monitoring)
        {
            ...
            rateidx = spinfo->real_hw_index;
        //printk("rc-%d-%d wsid[%d] %p index[%d]\n",spinfo->real_hw_index,rates[SSV_DRATE_IDX].count,rc_sta->rc_wsid,spinfo,sta_priv->rc_idx);
    }
    ...
    //printk("pre SSV_DRATE_IDX=%d-%d\n",rates[SSV_DRATE_IDX].count, rates[SSV_CRATE_IDX].count);
    ...
    if(rc_rate->hw_rate_idx != rates[SSV_DRATE_IDX].count)
    {
        ...
        /**
        * Because only single-rate is supported, we use rates[SSV_DRATE_IDX] as
        * the hardware rate index for data frames while rates[SSV_CRATE_IDX] as
        * the hardware rate index for control frames.
        *
        ...
        */
        ...
        ret |= 0x1;
        //printk("%s(): Flags=0x%x drate_idx=%d, crate_idx=%d\n", __FUNCTION__, rates[0].flags, rates[SSV_DRATE_IDX].count, rates[SSV_CRATE_IDX].count);
    }
