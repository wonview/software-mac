				    ...
				    struct ieee80211_key_conf *key)
{
	...
	vif_priv->is_security_valid = true;
	if (vif_priv->vif_idx == 1) {
		struct ssv_vif_priv_data *another_vif_priv =
		    (struct ssv_vif_priv_data *)sc->vif_info[0].vif_priv;
		if (another_vif_priv != NULL) {
			if (((vif_priv->has_hw_decrypt) &&
			     (another_vif_priv->need_sw_encrypt)) ||
			    ((vif_priv->need_sw_encrypt) &&
			     (another_vif_priv->has_hw_decrypt))) {
				u32 val;
				SMAC_REG_READ(sc->sh, ADR_RX_FLOW_DATA, &val);
				if (((val >> 4) & 0xF) != M_ENG_CPU) {
					SMAC_REG_WRITE(
			   ...
			   struct ieee80211_key_conf *key)
{
	...
	case DISABLE_KEY: {
		...
		if ((vif_info->if_type == NL80211_IFTYPE_STATION) &&
		    (sta == NULL)) {
			printk(" if type %d, if idx %d,sta %p\n",
			       vif_info->if_type, vif_priv->vif_idx, sta);
			if (another_vif_priv != NULL) {
				if ((vif_priv->need_sw_encrypt) &&
				    (another_vif_priv->has_hw_decrypt)) {
#ifdef SSV_SUPPORT_HAL
#ifdef CONFIG_SSV_HW_ENCRYPT_SW_DECRYPT
					ssv6xxx_set_rx_flow(
					    sc->sh, RX_DATA_FLOW, RX_HCI);
#else
					ssv6xxx_set_rx_flow(sc->sh,
							    RX_DATA_FLOW,
							    RX_CIPHER_HCI);
#endif
#else
...
#endif
	if (sh->cfg.ipd) {
		SMAC_REG_READ(sc->sh, ADR_ABB_REGISTER_1,
			      &sc->def_setting_CE01000C);
		SMAC_REG_READ(sc->sh, ADR_LDO_REGISTER,
			      &sc->def_setting_CE010008);
		SMAC_REG_READ(sc->sh, ADR_RX_ADC_REGISTER,
			      &sc->def_setting_CE010030);
	}
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 7, 0)
